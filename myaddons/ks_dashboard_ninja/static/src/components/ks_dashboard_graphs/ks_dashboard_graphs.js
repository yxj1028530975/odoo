/** @odoo-module **/
import { Component, onWillStart, useState ,useEffect,onMounted, onWillRender,useRef,onWillPatch, onRendered ,onPatched} from "@odoo/owl";
...
//                        width: am5.percent(80-(10*k)),
            ...
            if (item.ks_dashboard_item_type == 'ks_bullet_chart' || item.ks_dashboard_item_type == 'ks_radialBar_chart' || item.ks_dashboard_item_type === 'ks_flower_view' || item.ks_dashboard_item_type == 'ks_polarArea_chart' || item.ks_dashboard_item_type == 'ks_bar_chart' || item.ks_dashboard_item_type == 'ks_horizontalBar_chart'){
...
//            else if(item.ks_dashboard_item_type == 'ks_pie_chart'){
...
//                        $(self.$el.find('.ks_pager')).removeClass('d-none');
                ...
                if(!domain){
                $($(this.main_body.el).find(".grid-stack-item[gs-id=" + item_id + "]").children()[0]).find(".ks_dashboard_item_drill_up").addClass('d-none');
            }
    ...
    async ksrendermapview($ks_map_view_tmpl,item){
        ...
        if (map_data.groupByIds){
            ...
            await this._rpc("/web/dataset/call_kw/res.partner/search_read",{
                model: 'res.partner',
                method: 'search_read',
                args: [domain, fields],
                kwargs : {},
...
Ksdashboardgraph.template = "Ks_gridstack_container";
